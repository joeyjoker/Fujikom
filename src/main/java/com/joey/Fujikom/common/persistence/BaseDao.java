...
import com.joey.Fujikom.common.utils.Reflections;
...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
    ...
    public T findOne(DetachedCriteria detachedCriteria) {
        List<T> list = find(detachedCriteria, Criteria.DISTINCT_ROOT_ENTITY);
        if (list.size() > 0)
            return list.get(0);
        return null;
    }
     ...
     * @return
     */
    public List<T> findWithLimit(DetachedCriteria detachedCriteria, Integer pageSize) {
        findWithLimit(detachedCriteria, null, pageSize);
